...
const { success, notfound, failed } = require('../helpers/response');
...
const flight = {
    getAll: (req, res) => {
        try {
            ...
            flightModel.getAll(fromcity, tocity, typeflight, classflight, datedeparture, nameairlines, direct, transit, moretransit, departurefrom, departureto, arrivedfrom, arrivedto, pricefrom, priceto)
                .then((result) => {
                    if (result.length === 0) {
